...
bool CircleCollider::CollidesWith(const CircleCollider &circle) const {
  return CheckCollisionCircles(GetPosition(), GetRadius(), circle.GetPosition(),
                               circle.GetRadius());
}

bool CircleCollider::CollidesWith(const RectangleCollider &rect) const {
  return CheckCollisionCircleRec(GetPosition(), GetRadius(), rect.GetRect());
}
...
float CircleCollider::GetRadius() const { return radius; }

void CircleCollider::SetRadius(float radius) { this->radius = radius; }