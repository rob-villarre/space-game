...
Bullet::Bullet(Vector2 position, float heading, std::weak_ptr<Ship> owner) {
  this->texture =
      TextureManager::Load("src/assets/kenneyshmup/Tiles/tile_0000.png");
...
void Bullet::Draw() {
  ...
  DrawTexturePro(
      ...
      Rectangle{position.x, position.y, (float)texture.width,
                (float)texture.height},
      Vector2{(float)texture.width / 2.0f, (float)texture.height / 2.0f},
      heading, WHITE);
}

std::weak_ptr<Ship> Bullet::GetOwner() { return owner; }

Vector2 Bullet::GetPosition() { return position; }

float Bullet::GetHeading() { return heading; }

Rectangle Bullet::GetRect() {
  return Rectangle{position.x, position.y, (float)texture->width,
                   (float)texture->height};
}

CircleCollider &Bullet::GetCollider() { return *collider; }