...
RectangleCollider::RectangleCollider(Vector2 position, Vector2 size,
                                     float rotation)
...
bool RectangleCollider::CollidesWith(const CircleCollider &circle) const {
  return CheckCollisionCircleRec(circle.GetPosition(), circle.GetRadius(),
                                 GetRect());
}

bool RectangleCollider::CollidesWith(const RectangleCollider &rect) const {
  return CheckCollisionRecs(GetRect(), rect.GetRect());
}
...
Vector2 RectangleCollider::GetSize() const { return size; }

float RectangleCollider::GetRotation() const { return rotation; }

void RectangleCollider::SetSize(const Vector2 &size) { this->size = size; }
